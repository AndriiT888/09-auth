...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log("Set-Cookie header:", apiRes.headers["set-cookie"]);
    console.log("Response data:", JSON.stringify(apiRes.data, null, 2));
    console.log("All headers:", JSON.stringify(apiRes.headers, null, 2));
    ...
    // Фолбек: токен у body
    if (apiRes.data?.token || apiRes.data?.accessToken) {
      const cookieStore = await cookies();
    ...
    // Якщо немає ні cookie ні токенів — повертаємо дані як є
    return NextResponse.json(apiRes.data, { status: apiRes.status });
  } catch (error: unknown) {
